(function () {
    ...
    var CACHE_TTL = {
        ...
        recap:           7 * 864e5,
        search:          1 * 864e5,
        trailers:       14 * 864e5, // YouTube key стабільний, тримаємо довго
    };
    ...
    function AIAssistantPlugin() {
        ...
        this.openAiMenu = function (card, btnElement, renderContainer, prevCtrl) {
            ...
            var items = [
                ...
                { title: '\u0422\u0440\u0435\u0439\u043b\u0435\u0440\u0438', action: 'trailers' },
            ...
            Lampa.Select.show({
                ...
                onSelect: function (item) {
                    setTimeout(function () {
                        ...
                        else if (item.action === 'trailers')        _this.actionTrailers(card, btnElement, renderContainer, ctrl);
        ...
        // -------------------------------------------------------------------
        //  ACTION: TRAILERS
        //  TMDB videos endpoint. Мовний пріоритет: uk -> ru -> en (за запитом
        //  користувача: "якщо немає української, тоді мова окупанта, потім англійська").
        //  Тільки офіційні Trailer/Teaser з YouTube, найвища якість (size, потім official).
        // -------------------------------------------------------------------

        this.actionTrailers = function (card, btn, render, ctrl) {
            window.ai_active_controller = ctrl || Lampa.Controller.enabled().name;
            var method = (card.name || card.original_name) ? 'tv' : 'movie';

            // Кеш на трейлери — посилання на YouTube не змінюються, тримаємо довго
            var cached = _this.cacheGet('trailers', card.id);
            if (cached) { _this.showTrailerPicker(cached, card, btn, render, ctrl); return; }

            _this.updateStatus('\u041f\u043e\u0448\u0443\u043a \u0442\u0440\u0435\u0439\u043b\u0435\u0440\u0456\u0432');

            // TMDB videos endpoint не приймає language-фільтр напряму для відбору —
            // повертає ВСІ мовні версії одразу, фільтруємо самі за iso_639_1.
            var url = Lampa.TMDB.api(method + '/' + card.id + '/videos?api_key=' + Lampa.TMDB.key());

            Lampa.Network.silent(url, function (res) {
                _this.hideStatus();
                var all = res.results || [];

                // Тільки YouTube (Vimeo рідко доступний на старих Android WebView без додаткових кодеків)
                // і тільки реальні трейлери/тізери — не Clip/Featurette/Behind the Scenes
                var trailers = all.filter(function (v) {
                    return v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser');
                });

                if (!trailers.length) {
                    Lampa.Noty.show('\u0422\u0440\u0435\u0439\u043b\u0435\u0440\u0438 \u0434\u043b\u044f \u0446\u044c\u043e\u0433\u043e \u0444\u0456\u043b\u044c\u043c\u0443 \u043d\u0435 \u0437\u043d\u0430\u0439\u0434\u0435\u043d\u043e');
                    if (window.ai_active_controller) Lampa.Controller.toggle(window.ai_active_controller);
                    return;
                }

                // Мовний пріоритет: українська -> мова окупанта -> англійська -> решта
                // (за прямим запитом користувача в чаті)
                var LANG_PRIORITY = ['uk', 'ru', 'en'];
                var byLang = {};
                trailers.forEach(function (v) {
                    var lang = v.iso_639_1 || 'xx';
                    if (!byLang[lang]) byLang[lang] = [];
                    byLang[lang].push(v);
                });

                var orderedLangs = LANG_PRIORITY.filter(function (l) { return byLang[l]; })
                    .concat(Object.keys(byLang).filter(function (l) { return LANG_PRIORITY.indexOf(l) === -1; }));

                // Всередині кожної мови: official=true спочатку, потім вища роздільна здатність (size)
                var sortedTrailers = [];
                orderedLangs.forEach(function (lang) {
                    var group = byLang[lang].slice().sort(function (a, b) {
                        if (a.official !== b.official) return a.official ? -1 : 1;
                        return (b.size || 0) - (a.size || 0);
                    });
                    sortedTrailers = sortedTrailers.concat(group);
                });

                _this.cacheSet('trailers', card.id, sortedTrailers);
                _this.showTrailerPicker(sortedTrailers, card, btn, render, ctrl);

            }, function () {
                _this.hideStatus();
                Lampa.Noty.show('\u041f\u043e\u043c\u0438\u043b\u043a\u0430 \u0437\u0430\u0432\u0430\u043d\u0442\u0430\u0436\u0435\u043d\u043d\u044f \u0442\u0440\u0435\u0439\u043b\u0435\u0440\u0456\u0432');
                if (window.ai_active_controller) Lampa.Controller.toggle(window.ai_active_controller);
            });
        };

        // Карта мовних кодів для зрозумілого підпису користувачу
        var TRAILER_LANG_NAMES = {
            uk: '\u0423\u043a\u0440\u0430\u0457\u043d\u0441\u044c\u043a\u0430',
            ru: '\u0420\u043e\u0441\u0456\u0439\u0441\u044c\u043a\u0430',
            en: '\u0410\u043d\u0433\u043b\u0456\u0439\u0441\u044c\u043a\u0430',
            xx: '\u041d\u0435\u0432\u0456\u0434\u043e\u043c\u0430'
        ...
        this.showTrailerPicker = function (trailers, card, btn, render, ctrl) {
            // Якщо трейлер один — одразу відкриваємо, без зайвого меню вибору
            if (trailers.length === 1) {
                _this.playTrailer(trailers[0], btn, render, ctrl);
                return;
            }

            var items = trailers.slice(0, 10).map(function (t) {
                var langName = TRAILER_LANG_NAMES[t.iso_639_1] || t.iso_639_1.toUpperCase();
                var qualityTag = t.size >= 1080 ? ' (' + t.size + 'p)' : (t.size ? ' (' + t.size + 'p)' : '');
                var officialTag = t.official ? ' \u2605' : '';
                return {
                    title: langName + qualityTag + officialTag + ' \u2014 ' + (t.name || t.type),
                    trailer_data: t
                };
            });

            Lampa.Select.show({
                title: '\u041e\u0431\u0435\u0440\u0456\u0442\u044c \u0442\u0440\u0435\u0439\u043b\u0435\u0440',
                items: items,
                onSelect: function (item) { _this.playTrailer(item.trailer_data, btn, render, ctrl); },
                onBack: function () { _this.openAiMenu(card, btn, render, ctrl); }
            });
        };

        this.playTrailer = function (trailer, btnElement, renderContainer, controllerName) {
            // youtube-nocookie.com — privacy-friendly embed домен, той самий підхід
            // що рекомендує офіційна документація YouTube IFrame API для вбудованих плеєрів.
            // Працює в будь-якому WebView без залежності від внутрішнього API Lampa.Player,
            // яке може відрізнятись між форками і версіями додатку.
            var embedUrl = 'https://www.youtube-nocookie.com/embed/' + trailer.key + '?autoplay=1&rel=0&modestbranding=1';
            var html = '<div style="position:relative;width:100%;padding-top:56.25%;">' +
            ...
            _this.showViewer('\u0422\u0440\u0435\u0439\u043b\u0435\u0440: ' + (trailer.name || ''), html, btnElement, renderContainer, controllerName);
        };
