(function () {
    ...
    function AIAssistantPlugin() {
        ...
        this.buildTMDBContext = function (card, callback) {
            // Якщо контекст вже є в картці — повертаємо одразу (економія часу та мережі)
            if (card._ai_ctx) {
                dbg.log('TMDB Context: з кешу картки ✓');
                callback(card._ai_ctx);
                return;
            }
            ...
            Lampa.Network.silent(url, function (res) {
                ...
                // Зберігаємо контекст у картці — наступний виклик буде миттєвим
                card._ai_ctx = ctx;
        ...
        this.buildFactsPrompt = function (ctx) {
            return 'Уяви що розповідаєш другу цікаві речі про ' + ctx.type +
                ' "' + ctx.original_title + '" (' + ctx.year + '). Мова: українська.\n\n' +
                'Дай 7 фактів які справді здивують звичайного глядача. Не технічні деталі — \n' +
                'а речі які змушують сказати "ого, не знав!". Наприклад:\n' +
                '- Смішна або драматична історія що трапилась на знімальному майданчику\n' +
                '- Хто насправді надихнув головного героя (реальна людина чи подія)\n' +
                '- Відома сцена яку мало не вирізали або знімали зовсім інакше\n' +
                '- Несподіваний зв\'язок з іншим відомим фільмом або реальним фактом\n' +
                '- Щось про акторів або режисера що здивує фаната\n\n' +
                'ЗАБОРОНЕНО: дата виходу, перелік нагород, бюджет, "фільм вийшов у...".\n' +
                'Пиши живо і з захопленням, як розповідь другу.\n\n' +
                'Поверни ТІЛЬКИ JSON масив:\n' +
                '[{"title":"Коротка цікава назва","text":"2-3 речення живим стилем"}]';
        };

        this.buildRecapPrompt = function (itemTitle, franchiseTitle, year) {
            return 'Ти — кіноглядач що переказує другу що відбулося у "' + itemTitle +
                '" (' + franchiseTitle + ', ' + year + '). Мова: українська.\n\n' +
                'Напиши 10 коротких пунктів — ЩО відбулося, без будь-яких заголовків категорій.\n' +
                'Кожен пункт — одна чітка подія або факт. Розмовний стиль, зрозуміло для глядача.\n\n' +
                'ЗАБОРОНЕНО: заголовки типу "Основна сюжетна арка:", "Ключовий розвиток:", \n' +
                'категорії, нумерація типу "1.", повторення однакових вступних слів.\n\n' +
                'Поверни ТІЛЬКИ JSON масив з 10 пунктами:\n' +
                '[{"point":"..."}]';
        };
        ...
        this.setupSettings = function () {
            ...
            Lampa.SettingsApi.addParam({
                ...
                onRender: function (item) {
                    // Показуємо назву провайдера, прибираємо клікабельність
                    var name = _this.getActiveProviderName();
                    item.find('.settings-param__value').text(name).css('color', name === 'Недоступний' ? '#f55' : '#4b5');
                    item.find('.settings-param__name').text('Провайдер: ' + name);
                    item.css('cursor', 'default').off('hover:enter').off('click');
                    // Приховуємо value — назва вже в полі name
