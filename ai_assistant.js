    ...
    function AIAssistantPlugin() {
        ...
        this.cacheSet = function (action, id, data) {
            try {
                ...
                // QuotaExceededError: localStorage переповнено — видаляємо протухлі записи
                if (e.name === 'QuotaExceededError' || e.code === 22) {
                    dbg.log('Cache: QuotaExceededError — очищення протухлих записів');
                    try {
                        var prefix = 'ai_cache_';
                        var now = Date.now();
                        var keysToDelete = [];
                        for (var k in localStorage) {
                            if (k.indexOf(prefix) === 0) {
                                try {
                                    var entry = JSON.parse(localStorage[k]);
                                    var ttl = CACHE_TTL[k.split('_')[2]] || CACHE_TTL.recommendations;
                                    if (!entry || now - entry.timestamp > ttl) keysToDelete.push(k);
                                } catch (pe) { keysToDelete.push(k); }
                            }
                        }
                        keysToDelete.forEach(function(dk){ try { localStorage.removeItem(dk); } catch(re){} });
                        // Повторна спроба після очищення
                        Lampa.Storage.set(_this.cacheKey(action, id), JSON.stringify({
                            data: data, timestamp: now, version: PLUGIN_VERSION
                        }));
                        dbg.log('Cache: ' + keysToDelete.length + ' застарілих записів видалено, повторна спроба збереження OK');
                    } catch (re) { dbg.log('Cache: повторна спроба після очищення теж не вдалась:', re.message); }
                } else {
                    dbg.log('Cache save error:', e.message);
                }
            }
        };
